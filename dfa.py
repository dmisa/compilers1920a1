...
transitions = { 
        's0':{'DIGIT':'s1','.':'s2'}, #allagi2 's0':{'DIGIT':'s1','0':'s1','.':'s2'}
        's1':{'.':'s3','DIGIT':'s1','0':'s1'},
        's2':{'DIGIT':'s3','0':'s3'},
        's3':{'DIGIT':'s3'}
        
    # (Α) Επέλεξα να αποκλείσω το 0 από την αρχή του αριθμού. Καθαρά για λόγους ομορφιάς.
    # Αν θέλαμε και το 0 τότε στο DIGIT στην getchar() θα κάναμε την allagi1 ή την allagi2
              } 
...
def get_char(text,pos):
    ...
    c = text[pos]
    if c>'0' and c<='9': #allagi 1 : if c>='0' and c<='9'
            return 'DIGIT'
        # (Γ)
    return c
